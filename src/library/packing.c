...
void pack_ampphase_modifiers(cmp_ctx_t *cmp, struct ampphase_modifiers *a) {
  ...
  pack_write_bool(cmp, a->add_phase);
  pack_write_sint(cmp, a->phase_num_antennas);
  pack_write_sint(cmp, a->phase_num_pols);
  pack_write_double(cmp, a->phase_start_mjd);
  pack_write_double(cmp, a->phase_end_mjd);
...
void unpack_ampphase_modifiers(cmp_ctx_t *cmp, struct ampphase_modifiers *a) {
  ...
  pack_read_bool(cmp, &(a->add_phase));
  pack_read_sint(cmp, &(a->phase_num_antennas));
  pack_read_sint(cmp, &(a->phase_num_pols));
  pack_read_double(cmp, &(a->phase_start_mjd));
  pack_read_double(cmp, &(a->phase_end_mjd));
  if (a->phase_num_antennas > 0) {
    CALLOC(a->phase, a->phase_num_antennas);
    if (a->phase_num_pols > 0) {
      for (i = 0 ; i < a->phase_num_antennas; i++) {
	CALLOC(a->phase[i], a->phase_num_pols);
	pack_readarray_float(cmp, a->phase_num_pols, a->phase[i]);
      }
    }
  } else {
    a->phase = NULL;
