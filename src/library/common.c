...
void spectrum_data_compile_system_temperatures(struct spectrum_data *spectrum_data,
  ...
  *num_ants = tsys_data->num_ants;
  *num_ifs = tsys_data->num_ifs;
  *num_pols = tsys_data->num_pols;

  CALLOC(*system_temperatures, *num_ants);
  for (i = 0; i < *num_ants; i++) {
    CALLOC((*system_temperatures)[i], *num_ifs);
    for (j = 0; j < *num_ifs; j++) {
      CALLOC((*system_temperatures)[i][j], *num_pols);
